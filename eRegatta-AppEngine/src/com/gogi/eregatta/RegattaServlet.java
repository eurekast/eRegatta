...
public class RegattaServlet extends HttpServlet {
		...
		try{
			...
			if (!searchInputList.isEmpty()){													// èe najde search polje
			    ...
			    if (!cityDivList.isEmpty()){
			    	...
			    	//
			    	List<?> priceTrList = page3stars.getByXPath("//*[@id='search_results_table']/table/tbody/tr/td[2]/div[3]/form/table/tbody/tr");
			    	if (priceTrList.size()>0){
			    		...
			    		while(priceTrIterator.hasNext()){		
		    				try {
	    	    				...
	    	    				log.warning("DEBUG: max persons stripped content: "+maxPersons);
